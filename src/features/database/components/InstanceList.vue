    ...
    <Card v-if="showCreateForm" class="mb-4">
      <CardContent class="p-4 space-y-3">
        <h3 class="font-medium text-sm">
          Create {{ store.engineDisplayName(engine) }} Instance
        </h3>
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            >
              ...
              <option
                v-for="v in installedVersions[engine] || []"
                :key="v"
                :value="v"
              >
                {{ v }}
            ...
            <div class="flex items-center gap-2 shrink-0 ml-4">
              <template v-if="confirmingDelete === instance.instanceId">
                ...
                <Button
                  variant="destructive"
                  size="sm"
                  @click="onDelete(instance.instanceId)"
                >
                  Yes
                </Button>
                <Button
          ...
          <div v-if="instance.running" class="mt-3 pt-3 border-t border-border">
            <div class="flex items-center justify-between mb-2">
              ...
              >
                <Database
                  :class="
                    expandedDb === instance.instanceId
                      ? 'size-3.5 mr-1'
                      : 'size-3.5 mr-1 rotate-0'
                  "
                  class="size-3.5 mr-1"
                />
            ...
            <template v-if="expandedDb === instance.instanceId">
              <!-- Database List -->
              <div
                v-if="databases[instance.instanceId]?.length"
                ...
                >
                  ...
                  <div class="flex items-center gap-1 shrink-0 ml-2">
                    <button
                      ...
                      title="Export"
                      @click="onExportDatabase(instance.instanceId, db)"
                    >
              ...
              <p
                v-if="dbLoading[instance.instanceId]"
                class="text-xs text-muted-foreground py-2"
              >
                Loading...
              </p>
              <p
                v-else-if="!databases[instance.instanceId]?.length"
                class="text-xs text-muted-foreground py-2"
              >
                No user databases.
              </p>
              ...
              <div class="flex items-center gap-2">
                ...
                <Button
                  ...
                  :disabled="
                    !newDbName.trim() || creatingDb === instance.instanceId
                  "
                  @click="onCreateDatabase(instance.instanceId)"
                >
                  <Plus
                    v-if="creatingDb !== instance.instanceId"
                    class="size-3 mr-1"
                  />
...
async function onExportDatabase(instanceId: string, databaseName: string) {
  const instance = instances.value.find((i) => i.instanceId === instanceId);
  const defaultName = instance
    ? `${instance.displayName || instance.engine}-${databaseName}.sql`
    : `${databaseName}.sql`;
  ...
  if (!filePath) return;
  try {
    await store.exportDatabase(instanceId, databaseName, filePath);
  } catch {}
}

async function onImport(instanceId: string) {
  const filePath = await window.electronAPI.showOpenDialog({
    filters: [{ name: "SQL Files", extensions: ["sql"] }],
  });

  if (!filePath) return;
  const databaseName = prompt("Database name to import into:");
  if (!databaseName) return;
  try {
    await store.importDatabase(instanceId, filePath, databaseName);
    await loadDatabases(instanceId);
  } catch {}
}
</script>
